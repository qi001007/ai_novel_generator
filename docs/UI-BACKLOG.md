...
## 一、2026-09-05 第十一轮批注（9 条 + 1 句主请求，全部落地）

结论逐条在 `WORKSTREAM-PLAN.md` 第十一批。本轮只留一条待主人看的观察：

- [ ] **对话里那条 `[TOOL_CALL]` 是历史脏数据，不是现行缺陷**：消息 id=20 存于
      2026-09-04 **05:04**，而修方言过滤的 `93ec9ee` 是同日 **13:06**——早了 8 小时。
      前端现在无论如何都会把它折进「思考过程」，所以不会再上屏；
      要不要顺手清掉那条旧行，主人说了算（它属于删除语义那条决策）。
