...
- **补记 · 记录册的主键写法**：分册沿用 `弧 N` 的机制——`## 伏笔 ? 标题` 里的 `?` 表示「由数据库分配」，人类与 AI 都不能改已分配的主键号。`已确认` 这类布尔渲染成 `是 / 否`，解析回来必须是 `bool`：留成文本会把「否」当真值写进库。世界观的 JSON 写口 `POST/PUT /settings` 前端从未调用，按 D-01 应转 410，**尚未转**（下一步做，先记录）。
