...
最后更新：2026-09-03，对应提交 `d9eb6a9`。所有数字均为本机实测，不是转述。
...
目标态（B 路，D-01）：一条入口，四层规划的任何修改都走它。
...
```text
左树点 0042.md ─► /files/briefs/0042.md  ─► documents.write_file ─► ChapterBrief 行
树底新建章节  ─► /planning/briefs POST   ─► routers.planning     ─► ChapterBrief 行  ← 同一张表
```

- **DB 是唯一真源**（D-02）。`app/models.py` 里四层规划全是结构化列，库里不存文档文本；
  文件内容由 `services/documents.py` 每次现渲染、解析回列。换文件格式属纯投影层改动（零迁移、零 alembic）。
- 两套写通路的**校验强度不同**：文件层有键锁（D-10）、`actor=ai` 白名单、`base_revision` 乐观并发（409）、
  `stabilize_proposal`；`/planning/*` 一个都没有。**从 A 路改可以绕过 B 路全部保护**。
- 现在还没炸，是因为 brief 恰好在 A 路只用于「建」、在 B 路用于「改」。**一旦对话 Agent 开始管规划就会炸**
  —— 它两条都会碰。这是 S3 排在 S2 之前的唯一理由。
...
toc.md                  B 目录
arcs.md                 C 剧情弧
chapters/0042/brief.md  D 单章简报   ← 现状是 briefs/0042.md
chapters/0042/draft.md  正文         ← 现状正文不是文件，只在 Chapter.content
```
...
| `services/documents.py` + `markdown_doc.py` | 538 + 291 | MD 投影读写、键锁、AI 白名单、乐观并发、提案稳定化 | 可用，待 D-01 / D-03 改造 |
...
实测门禁：后端 **101 passed / 3.0s**，前端 **51 passed / 12 files**，`npm run build` 干净。
...
| 3 | 两套写通路未收口 | 见 §1 | S3 |
| 4 | 正文不是文件，无 `chapters/NNNN/draft.md` | `EditorPane.tsx` 直接编辑 `Chapter.content` 字段 | S3 / S1 |
...
当前进度：**S0 代码就绪待演示 → S1 待办 → S3 待办 → S2 未开始 → S4 未开始**。逐项勾选在 `WORKSTREAM-PLAN.md`。
