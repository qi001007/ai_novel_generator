...
- [ ] Figma 逐弹层/逐状态画稿（主人已提出可解除 MCP 限额，开通后按 v3 清单补画：对话输入坞三态、模式切换、模型菜单、附件 chip、框选工具条、双栏修改页、A-D 文档页、目录树、人物卡/弹层、绘画详情页、伏笔墙、反馈时间线）
- [ ] 框选 → AI 修改 → 双栏对照页（等 Figma 稿与 C5 修改接口，UI 壳可先做）
- [ ] Ctrl+F 查找条与全量快捷键接线（随 TipTap 编辑器迁移一起做）
- [ ] 反馈时间线页（替代现 FeedbackPanel）与 AI 自检主动提示设定冲突
- [ ] 世界观/地图/伏笔页面壳与空态
