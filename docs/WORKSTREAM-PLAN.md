...
- [x] **第十五批·批注 3.1（2026-09-05）：人物文件渲染成人物卡片**：
      §3.2 那次把「按 kind 选渲染视图」收进 `renderedViewLabel()`，这一条补上它的第二项。
      新增 `CharacterDocCard.tsx`：`parseCharacterDoc()` 读结构行（`- **姓名**：…`）与四个
      `## ` 长字段；**小节里的 bullet 是作者的正文，不是字段**——按这个规则解析，
      否则「## 目标」下的列表会被当成结构而丢掉半张卡。
      覆盖层与目录列表同一规矩 `inset: 0`（不写魔法偏移），`showRendered` 明确排除人物路径；
      分级色与分级词从 `CharacterLibrary` 导出 `LEVEL_LABELS` 共用，不留第二份表。
      真机就用主人点名的 `novels/5/settings/characters/6.md`：卡片 560x346、
      沈砚舟／主角团／1 - 12 章／身份·目标·行为约束·当前状态 全在、结构行未上屏；
      点钮回源码、再点回卡片；**未保存的字照样上卡片**（老规矩复验）。
      测完把 buffer 还原，服务器那份未写入（`len 210 / rev cc375383eb90` 未变）。
