...
当前进行到：**S0 代码已就绪，待跑给主人看一次**（2026-09-03 主干重排后的起点）。

- 后端 101 passed / 前端 51 passed / build 干净（2026-09-03 实测）。
...
- [ ] **跑一次给主人看**：`NOVEL_CONTEXT_DEBUG=1` 生成一章，终端打印清单
- [ ] 主人据清单指认「哪个 kind 该删 / 该加」，形成第一批 collector 调整
...
- [ ] 先写测试：`/planning/*` 的 POST/PUT 不可再用；`resolve_path` 新旧路径双向可解析
- [ ] 新建章节改走文件层：一次同时建 `Chapter` 与 `chapters/{N}/brief.md`
      （现走 `POST /planning/briefs`，见 `frontend/src/store/workbench.ts:164`）
- [ ] 文件层补「新建文件」端点（B 路唯一的建条目入口）
- [ ] `resolve_path` 支持 `chapters/0042/brief.md`，**读旧写新**，旧 `briefs/0042.md` 仍可读
- [ ] 正文投影成 `chapters/0042/draft.md`（`EditorPane` 与文件层同一条写通路）
- [ ] `routers/planning.py` 退役写端点，只读 GET 按需保留
- [ ] 前端 `store/files.ts` 路径常量与左树 `briefs/` 分组换成按章归组
...
### U2 帧 21：B 层目录列表视图与搜索（**待主人审批，批前不写代码**）

- [x] 帧 21 已出（节点 `460:6`），自查修掉两处缺陷：保存按钮溢出 596 宽面板、控件内文字底部切掉
- [ ] 批准后实现：列表视图 + 行内改名即存 + 本地搜索 + 源码开关（两视图共用一份数据，不产生第二真源）

### U3 帧 22：绘画详情独立页（**待审批**）

- [x] 帧 22 已出（节点 `464:6`），假模型名已改为「文生图模型（待接）」
- [ ] 批准后实现（后端生图能力 = 0，见缺口清单；先只做页面壳与 token 明细）
...
- [ ] Q-01：19 个只被旧表单视图用过的死样式类
- [ ] Q-02：novel 3「MD探针」测试作品——需先决定是否加 `DELETE /api/novels/{id}`
- [ ] Q-03：帧 20 未落的 4 项规格装饰
- [ ] Q-04：人物页分级 Tab 与帧 08 的 chip 形态不一致
