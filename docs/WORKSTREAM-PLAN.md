...
- [ ] **批注 6–21 转代码批次**（真机既有控件，按 UI 铁律不需新帧，逐条量过再改）。我按依赖排序：
      ...
      **进度（2026-09-04 本轮）：1–13 与 15–21 已全部落地并真机复验，共 8 个提交；只剩 14 与 20。**
...
- [ ] **批注 20 未做**：章节渲染视图要能在顶栏多开、直接切换、可关闭（与上一轮批注 9 是同一条，
      主人 2026-09-04 再次点名「我说过了」）。需要给章节建 tab 模型（现只有 `selectedChapterId`
      单值），与文件层 `useFiles.tabs` 并存，改动面较大，单独一轮做。
- [ ] **批注 14 未做**：对话栏 / 编辑栏 / 侧栏三栏各自可收起。入口按 §0.7 条二做成图标，不做长条。
